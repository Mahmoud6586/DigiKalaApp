...
import ir.example.digikalaapp.ui.component.OurLoading
...
fun SubCategorySection(viewModel: CategoryViewModel = hiltViewModel()) {
    ...
    when (subCategoryResult) {
        ...
        is NetworkResult.Error -> {
            loading = false
            Log.e("3636", "CategorySection error : ${subCategoryResult.message}")
        }
    }
    ...
    if(loading){
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            CategoryItem(title = stringResource(id = R.string.beauty_and_health), subList = beautyList)
            CategoryItem(title = stringResource(id = R.string.home_and_kitchen), subList = homeList)
            CategoryItem(title = stringResource(id = R.string.books_and_stationary), subList = bookList)
            CategoryItem(title = stringResource(id = R.string.sports_and_travel), subList = sportList)
        }
    }
